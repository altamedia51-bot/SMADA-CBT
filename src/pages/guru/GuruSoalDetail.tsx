...
`;
  ...
  const handleAddManual = async (e: React.FormEvent) => {
    ...
    if (soalType === 'pg' || soalType === 'pgk' || soalType === 'pdf') {
      ...
      if (benarSalahStatements.length === 0) return toast.error('Tambahkan minimal 1 pernyataan');
      if (benarSalahStatements.some(s => !s.statement.trim())) {
        return toast.error('Semua teks pernyataan wajib diisi');
      }
      if (benarSalahStatements.some(s => !s.answer)) {
        return toast.error('Semua pernyataan wajib ditentukan Benar atau Salah');
      }
      finalAnswer = 'multi-statement'; // Standardized marker for multi-statement scoring
