...
export default function InputNilaiKokurikuler({ isAdmin = false }: { isAdmin?: boolean }) {
  ...
  const loadData = async () => {
     ...
     try {
       ...
       const nilaiRef = doc(db, 'nilai_kokurikuler', getDocId());
  ...
  const saveAll = async () => {
     ...
     setLoading(true);
     try {
        await setDoc(doc(db, 'nilai_kokurikuler', getDocId()), { nilai: nilaiData, updatedBy: profile?.uid, updatedAt: new Date() }, { merge: true });
        toast.success("Berhasil menyimpan data nilai kokurikuler.");
     } catch (e: any) {
  ...
  return (
    <div className="p-4 md:p-8 space-y-6">
      <div>
        <h1 className="text-2xl font-black text-slate-800 uppercase tracking-tighter">Input Nilai Kokurikuler</h1>
        <p className="text-sm text-slate-500 font-medium">{isAdmin ? 'Pilih kelas untuk menginput nilai kokurikuler (Deep Learning / P5).' : 'Input nilai kokurikuler untuk kelas perwalian Anda.'}</p>
      </div>
                    ...
                    <Table className="min-w-max">
                       <TableHeader className="bg-slate-100 border-b-2">
                          <TableRow>
                             ...
                             <TableHead className="text-center border-x bg-blue-50 text-blue-800" colSpan={3}>KOKURIKULER 1</TableHead>
                             <TableHead className="text-center border-x bg-fuchsia-50 text-fuchsia-800" colSpan={3}>KOKURIKULER 2</TableHead>
                             <TableHead className="text-center border-x bg-emerald-50 text-emerald-800" colSpan={3}>KOKURIKULER 3</TableHead>
                          </TableRow>
                          <TableRow>
                             {[0,1,2].map(i => (
                                <React.Fragment key={`hdr-${i}`}>
                                   <TableHead className="text-center text-xs p-2">Nama Proyek</TableHead>
                                  ...
                                  {[0,1,2].map(i => (
                                     <React.Fragment key={`input-${siswa.id}-${i}`}>
                                        <TableCell className="p-1 border-x">
                                           <Input className="h-8 text-[11px]" value={scores[i]?.nama || ''} placeholder="Nama Kokurikuler..." onChange={e => handleChange(siswa.id, i, 'nama', e.target.value)} />
                                        </TableCell>
                                        <TableCell className="p-1 border-x">
                                           <Select value={scores[i]?.predikat || 'none'} onValueChange={v => handleChange(siswa.id, i, 'predikat', v === 'none' ? '' : v)}>
                                             ...
                                             <SelectContent>
                                                ...
                                                <SelectItem value="Berkembang Sesuai Harapan">Berkembang Sesuai </SelectItem>
